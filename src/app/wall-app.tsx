...
import { createCommentAction, createMemoryAction, createReportAction, deleteCommentAction, deleteMemoryAction, getActivityAction, getAllMemoriesAction, getCommunityDataAction, getRecentlyAddedAction, listCommentsAction, moderateCommentAction, searchMemoriesAction, setActivityPreferenceAction, updateMemoryAction, updatePlacementAction, type WallData } from "@/server/actions";
...
export function WallApp({ initialData }: { initialData: WallData }) {
  ...
  const [visibilityFilter, setVisibilityFilter] = useState<"all" | "private" | "selected-community">("all");
...
function DetailsPanel({ selected, selectedIsOwned, communities, editing, editForm, setEditForm, busy, positionMode, comments, commentDraft, setCommentDraft, commentsBusy, reportReason, setReportReason, onSubmitComment, onDeleteComment, onModerateComment, onReport, onStartPosition, onMovePosition, onConfirmPosition, onCancelPosition, onEdit, onSaveEdit, onCancelEdit, onDelete }: {
  ...
  return <aside id="memory-details" tabIndex={-1} aria-label="Memory details" className="border-t border-[#4f453f]/60 bg-[#1b1c15] p-5 outline-none md:min-h-[calc(100vh-72px)] md:border-l md:border-t-0 md:p-6">
    ...
    {!selected ? <div className="mt-16 text-center"><div aria-hidden="true" className="text-4xl text-[#665440]">⌁</div><h2 className="font-editorial mt-4 text-2xl text-[#d2c4bb]">Choose a memory</h2><p className="mt-2 text-sm leading-6 text-[#a8a79b]">Select a card to read its full reflection and manage it.</p></div>
      : editing ? <form onSubmit={onSaveEdit} className="mt-6 space-y-4"><label className="block text-xs text-[#a8a79b]">Title<input required maxLength={120} value={editForm.title} onChange={(event) => setEditForm({ ...editForm, title: event.target.value })} className="mt-1 w-full border-b border-[#806e58] bg-transparent py-2 text-lg text-[#e4e3d7] outline-none focus:border-[#e9c349]" /></label><label className="block text-xs text-[#a8a79b]">Reflection<textarea required maxLength={5000} rows={8} value={editForm.reflection} onChange={(event) => setEditForm({ ...editForm, reflection: event.target.value })} className="mt-1 w-full resize-y border-b border-[#806e58] bg-transparent py-2 text-sm leading-6 outline-none" /></label><CategorySelect value={editForm.category} onChange={(value) => setEditForm({ ...editForm, category: value })} /><label className="block text-xs text-[#a8a79b]">Visibility<select value={editForm.visibility} onChange={(event) => setEditForm({ ...editForm, visibility: event.target.value as "private" | "selected-community", communityIds: event.target.value === "private" ? [] : editForm.communityIds })} className="mt-1 w-full rounded border border-[#665c50] bg-[#292b23] px-3 py-2.5 text-sm"><option value="private">Private</option><option value="selected-community">Selected community</option></select></label>{editForm.visibility === "selected-community" && <fieldset className="rounded border border-[#665c50] p-3"><legend className="px-1 text-xs text-[#a8a79b]">Share with</legend>{communities.length ? communities.map((community) => <label key={community.communityId} className="flex items-center gap-2 py-1 text-sm"><input type="checkbox" checked={editForm.communityIds.includes(community.communityId)} onChange={(event) => setEditForm({ ...editForm, communityIds: event.target.checked ? [...editForm.communityIds, community.communityId] : editForm.communityIds.filter((id) => id !== community.communityId) })} />{community.name}</label>) : <p className="text-xs text-[#a8a79b]">You are not a member of a shareable community yet.</p>}</fieldset>}<div className="flex gap-2"><button disabled={busy} className="rounded-lg bg-[#e9c349] px-4 py-2 text-sm font-semibold text-[#3c2f00]">{busy ? "Saving…" : "Save changes"}</button><button type="button" onClick={onCancelEdit} className="rounded-lg border border-[#665c50] px-4 py-2 text-sm text-[#d2c4bb]">Cancel</button></div></form>
