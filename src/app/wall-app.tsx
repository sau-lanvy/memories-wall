...
import { createCommentAction, createMemoryAction, createReportAction, deleteCommentAction, deleteMemoryAction, getActivityAction, getAllMemoriesAction, getCommunityDataAction, getRecentlyAddedAction, listCommentsAction, moderateCommentAction, searchMemoriesAction, updateMemoryAction, updatePlacementAction, type WallData } from "@/server/actions";
import type { ActivityNotification, CommunityMembership, MemoryComment } from "@/domain/memory";

const STORAGE_KEY = "memories-wall:demo-user:personal";
const initialForm = { title: "", reflection: "", category: "gratitude" as MemoryCategory, visibility: "private" as "private" | "selected-community", communityIds: "" };
type FormValues = typeof initialForm;
type View = "wall" | "mine" | "all" | "community" | "recent";
...
export function WallApp({ initialData }: { initialData: WallData }) {
  ...
  const [editForm, setEditForm] = useState({ title: "", reflection: "", category: "gratitude" as MemoryCategory, visibility: "private" as "private" | "selected-community", communityIds: [] as string[] });
  ...
  const visibleMemories = useMemo(() => displayedMemories.filter((memory) => category === "all" || memory.category === category), [displayedMemories, category]);
  ...
  useEffect(() => {
    if (isOwnedView) { setSurfaceMemories(null); setSurfaceError(null); return; }
    setSurfaceLoading(true); setSurfaceError(null);
    void (async () => {
      if (view === "community") {
        const result = await getCommunityDataAction(communityId || undefined);
        if (!result.ok) { setSurfaceError(result.error); return; }
        setSurfaceMemories(result.data.memories); setCommunities(result.data.communities); return;
      }
      const result = view === "all" ? await getAllMemoriesAction() : await getRecentlyAddedAction();
      if (!result.ok) { setSurfaceError(result.error); return; }
      setSurfaceMemories(result.data);
    })().catch(() => setSurfaceError("This surface could not be loaded. Please try again.")).finally(() => setSurfaceLoading(false));
  }, [view, communityId, isOwnedView]);

  useEffect(() => {
    if (!selected || selected.visibility !== "selected-community") { setComments([]); return; }
    void listCommentsAction(selected.id).then((result) => { if (result.ok) setComments(result.data); });
  ...
  function reportMemory() {
    if (!selected) return;
    void createReportAction({ targetType: "memory", targetId: selected.id, reason: "other" }).then((result) => {
  ...
  return <div className="min-h-screen wood-grain wall-shadow">
    ...
    <div className="mx-auto grid max-w-[1600px] grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)_300px]">
      ...
      <main className="min-w-0 p-4 md:p-8">
        ...
        {view === "all" && <form className="mb-4 flex gap-2" onSubmit={(event) => { event.preventDefault(); if (!search.trim()) return; setSurfaceLoading(true); void searchMemoriesAction(search).then((result) => { if (result.ok) setSurfaceMemories(result.data); else setSurfaceError(result.error); }).finally(() => setSurfaceLoading(false)); }}><input aria-label="Search memories" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search your authorized memories" className="min-w-0 flex-1 rounded border border-[#665c50] bg-[#1b1c15] px-3 py-2 text-sm" /><button className="rounded border border-[#806e58] px-4 py-2 text-sm text-[#f5d97e]">Search</button></form>}
      ...
      <DetailsPanel selected={selected} selectedIsOwned={Boolean(selectedIsOwned)} editing={editing} editForm={editForm} setEditForm={setEditForm} busy={busy} positionMode={positionMode} comments={comments} commentDraft={commentDraft} setCommentDraft={setCommentDraft} commentsBusy={commentsBusy} onSubmitComment={submitComment} onDeleteComment={removeComment} onModerateComment={moderateCommentFromWall} onReport={reportMemory} onStartPosition={startPositionMode} onMovePosition={movePosition} onConfirmPosition={confirmPosition} onCancelPosition={() => setPositionMode(null)} onEdit={beginEdit} onSaveEdit={saveEdit} onCancelEdit={() => setEditing(false)} onDelete={() => setConfirmDelete(true)} />
    </div>
...
function DetailsPanel({ selected, selectedIsOwned, editing, editForm, setEditForm, busy, positionMode, comments, commentDraft, setCommentDraft, commentsBusy, onSubmitComment, onDeleteComment, onModerateComment, onReport, onStartPosition, onMovePosition, onConfirmPosition, onCancelPosition, onEdit, onSaveEdit, onCancelEdit, onDelete }: {
  ...
  onSubmitComment: (event: FormEvent<HTMLFormElement>) => void; onDeleteComment: (id: string) => void; onModerateComment: (id: string) => void; onReport: () => void;
  onStartPosition: () => void; onMovePosition: (x: number, y: number) => void; onConfirmPosition: () => void; onCancelPosition: () => void; onEdit: () => void; onSaveEdit: (event: FormEvent<HTMLFormElement>) => void; onCancelEdit: () => void; onDelete: () => void;
}) {
  return <aside id="memory-details" tabIndex={-1} aria-label="Memory details" className="border-t border-[#4f453f]/60 bg-[#1b1c15] p-5 outline-none md:min-h-[calc(100vh-72px)] md:border-l md:border-t-0 md:p-6">
    <div className="flex items-center justify-between"><p className="font-archive text-[10px] uppercase tracking-[.18em] text-[#a8a79b]">Details</p>{selected && <span className="rounded-full border border-[#6b855c] px-2 py-1 text-[10px] text-[#b6d0aa]">{selected.visibility === "private" ? "Private" : "Selected community"}</span>}</div>
    {!selected ? <div className="mt-16 text-center"><div aria-hidden="true" className="text-4xl text-[#665440]">⌁</div><h2 className="font-editorial mt-4 text-2xl text-[#d2c4bb]">Choose a memory</h2><p className="mt-2 text-sm leading-6 text-[#a8a79b]">Select a card to read its full reflection and manage it.</p></div>
      : editing ? <form onSubmit={onSaveEdit} className="mt-6 space-y-4"><label className="block text-xs text-[#a8a79b]">Title<input required maxLength={120} value={editForm.title} onChange={(event) => setEditForm({ ...editForm, title: event.target.value })} className="mt-1 w-full border-b border-[#806e58] bg-transparent py-2 text-lg text-[#e4e3d7] outline-none focus:border-[#e9c349]" /></label><label className="block text-xs text-[#a8a79b]">Reflection<textarea required maxLength={5000} rows={8} value={editForm.reflection} onChange={(event) => setEditForm({ ...editForm, reflection: event.target.value })} className="mt-1 w-full resize-y border-b border-[#806e58] bg-transparent py-2 text-sm leading-6 outline-none" /></label><CategorySelect value={editForm.category} onChange={(value) => setEditForm({ ...editForm, category: value })} /><label className="block text-xs text-[#a8a79b]">Visibility<select value={editForm.visibility} onChange={(event) => setEditForm({ ...editForm, visibility: event.target.value as "private" | "selected-community" })} className="mt-1 w-full rounded border border-[#665c50] bg-[#292b23] px-3 py-2.5 text-sm"><option value="private">Private</option><option value="selected-community">Selected community</option></select></label><div className="flex gap-2"><button disabled={busy} className="rounded-lg bg-[#e9c349] px-4 py-2 text-sm font-semibold text-[#3c2f00]">{busy ? "Saving…" : "Save changes"}</button><button type="button" onClick={onCancelEdit} className="rounded-lg border border-[#665c50] px-4 py-2 text-sm text-[#d2c4bb]">Cancel</button></div></form>
      : <div className="mt-6"><div className="mb-5 border-l-4 pl-4" style={{ borderColor: categoryMeta[selected.category].color }}><span className="text-xs font-semibold" style={{ color: categoryMeta[selected.category].color }}>{categoryMeta[selected.category].icon} {categoryMeta[selected.category].label}</span><h2 className="font-editorial mt-2 text-3xl leading-tight text-[#e4e3d7]">{selected.title}</h2></div><p className="whitespace-pre-wrap text-sm leading-7 text-[#d2c4bb]">{selected.reflection}</p><dl className="mt-8 space-y-3 border-t border-[#4f453f] pt-4 text-xs"><div className="flex justify-between gap-3"><dt className="text-[#a8a79b]">Written</dt><dd>{formatDate(selected.createdAt)}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#a8a79b]">Last updated</dt><dd>{formatDate(selected.updatedAt)}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#a8a79b]">Visibility</dt><dd>{selected.visibility === "private" ? "Private" : "Selected community"}</dd></div></dl>
        {selected.visibility === "selected-community" && <section className="mt-7 border-t border-[#4f453f] pt-4"><div className="flex items-center justify-between"><h3 className="font-editorial text-xl text-[#e4e3d7]">Responses <span className="font-archive text-xs text-[#a8a79b]">{comments.length}</span></h3><button type="button" onClick={onReport} className="text-xs text-[#ffb4ac] underline underline-offset-4">Report</button></div><div className="mt-3 space-y-3">{comments.map((comment) => <article key={comment.id} className="rounded border border-[#4f453f] bg-[#292b23] p-3"><p className="text-sm leading-5 text-[#d2c4bb]">{comment.body}</p><div className="mt-2 flex justify-between text-[10px] text-[#a8a79b]"><span>{formatDate(comment.createdAt)}</span><span className="flex gap-2">{comment.authorId === "demo-user" && <button type="button" disabled={commentsBusy} onClick={() => onDeleteComment(comment.id)} className="underline">Delete</button>}{selectedIsOwned && <button type="button" disabled={commentsBusy} onClick={() => onModerateComment(comment.id)} className="text-[#ffb4ac] underline">Remove</button>}</span></div></article>)}</div><form onSubmit={onSubmitComment} className="mt-4 flex gap-2"><input aria-label="Write a comment" value={commentDraft} onChange={(event) => setCommentDraft(event.target.value)} placeholder="Offer a thoughtful response" className="min-w-0 flex-1 rounded border border-[#665c50] bg-[#13140d] px-3 py-2 text-sm" /><button disabled={commentsBusy || !commentDraft.trim()} className="rounded bg-[#e9c349] px-3 py-2 text-xs font-semibold text-[#3c2f00]">Share</button></form></section>}
