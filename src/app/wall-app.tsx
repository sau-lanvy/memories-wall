...
function ImageWithFallback({ image, alt, className }: { image: MemoryImage; alt: string; className?: string }) {
  ...
  return <img src={image.thumbnailKey ?? image.storageKey} alt={alt} className={className} onError={() => setFailed(true)} />;
}
...
function MemoryGallery({ memory, owner, onAddImages, onRemoveImage }: { memory: Memory; owner: boolean; onAddImages: (event: FormEvent<HTMLFormElement>) => void; onRemoveImage: (id: string) => void }) {
  ...
  return <section className="memory-gallery mt-5" aria-label="Memory image gallery">
    {images.length > 0 && <div className="gallery-carousel" role="region" aria-roledescription="carousel" aria-label={`${memory.title} images`}>
      <div className="relative overflow-hidden rounded-xl border border-[#e5e1d8] bg-[#f4f1ea]"><div className="aspect-[4/3] w-full">{current && <ImageWithFallback image={current} alt={`${memory.title}, image ${index + 1} of ${images.length}`} className="h-full w-full object-cover" />}</div><span className="absolute bottom-2 right-2 rounded-full bg-black/65 px-2 py-1 text-[10px] text-white" aria-label={`Image ${index + 1} of ${images.length}`}>Image {index + 1} of {images.length}</span>{images.length > 1 && <><button type="button" aria-label="Previous image" onClick={() => setIndex((value) => (value - 1 + images.length) % images.length)} className="absolute left-2 top-1/2 rounded-full bg-white/90 px-2 py-1">‹</button><button type="button" aria-label="Next image" onClick={() => setIndex((value) => (value + 1) % images.length)} className="absolute right-2 top-1/2 rounded-full bg-white/90 px-2 py-1">›</button></>}</div>
